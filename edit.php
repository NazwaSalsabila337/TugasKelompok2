...
if (isset($_GET["id"]) && !empty($_GET["id"])) {
    $id = $_GET["id"];
    // var_dump($id);
    $sql = $con->prepare("SELECT * FROM sistem_prncatatan_penjualan WHERE id=?");
    $sql->bind_param("i", $id);
    $sql->execute();
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // var_dump($_POST);
        $sqls = $con->prepare("UPDATE sistem_prncatatan_penjualan SET name_product=?,price_product=?,updated_at=? WHERE id=?");
